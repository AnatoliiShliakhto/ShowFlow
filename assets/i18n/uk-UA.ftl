language-change = Змінити мову
language-en-US = [EN] Англійська
language-uk-UA = [UA] Українська

theme-change = Змінити тему
theme-light = Світла
theme-dark = Темна
theme-cupcake = Кекс
theme-bumblebee = Джміль
theme-emerald = Смарагдова
theme-corporate = Корпоративна
theme-synthwave = Сінтетична
theme-retro = Ретро
theme-garden = Сад
theme-forest = Ліс
theme-aqua = Аква
theme-lofi = Лофі
theme-pastel = Пастель
theme-fantasy = Фантазія
theme-wireframe = Каркас
theme-black = Чорна
theme-luxury = Розкіш
theme-dracula = Дракула
theme-cmyk = CMYK
theme-autumn = Осінь
theme-business = Бізнес
theme-acid = Кислота
theme-lemonade = Лимонад
theme-night = Ніч
theme-coffee = Кава
theme-winter = Зима
theme-dim = Тьмяна
theme-nord = Північ
theme-sunset = Захід сонця
theme-abyss = Безодня
theme-silk = Шовк

menu-title = Меню
menu-play = Показ
menu-playlist = Список показу
menu-info = Інформація
menu-exit = Вихід

message-finish = Дякуємо за увагу!
message-loading = Завантаження...
message-choose-folder = Вкажіть папку з презентаціями

title-playlist = Черга відтворення
title-files = Список файлів

action-play = РОЗПОЧАТИ
action-skip = НАСТУНИЙ
action-restart = З ПОЧАТКУ